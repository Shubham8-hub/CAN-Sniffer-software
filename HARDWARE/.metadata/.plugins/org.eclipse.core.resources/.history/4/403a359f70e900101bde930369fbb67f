...
//		CDC_Transmit_FS((uint8_t []) {RESP_ACK, dev_info.device_id, dev_info.fw_major, dev_info.fw_minor}, 4);

		// Here i want to add data from device_info how to do it
//	    uint8_t serial_len = strlen(dev_info.serial);
//	    uint8_t resp[32];
//
//	    resp[0] = RESP_ACK;
//	    resp[1] = dev_info.device_id;
//	    resp[2] = dev_info.fw_major;
//	    resp[3] = dev_info.fw_minor;
//	    resp[4] = dev_info.channel;
//	    resp[5] = serial_len;
//
//	    memcpy(&resp[6], dev_info.serial, serial_len);
...
//	    CDC_Transmit_FS(resp, 6 + serial_len);
